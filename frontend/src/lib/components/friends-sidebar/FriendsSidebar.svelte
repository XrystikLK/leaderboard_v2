...
>
	...
	<Sidebar.Content class="p-3">
		...
		{:else if userFriends.data && userFriends.data.length > 0}
			<Sidebar.Group>
				<Sidebar.GroupContent
					class="flex flex-col gap-2 overflow-y-auto max-h-[calc(100vh-6rem)] pr-1"
				>
					<Sidebar.Menu>
						{#each userFriends.data as friend (friend.steam_id)}
							<Sidebar.MenuItem>
								<div
									class="group/friend relative rounded-xl bg-sidebar-accent/30 hover:bg-sidebar-accent border border-sidebar-border p-3 transition-all duration-200 flex items-center justify-between gap-3 w-full"
								>
